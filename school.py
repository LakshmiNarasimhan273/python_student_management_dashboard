...
main = Tk()
main.title('School Management System by(Lakshmi Narasimhan)')
main.geometry('1250x650')
main.resizable(0, 0)

# Creating the background and foreground color variables
lf_bg = '#132226' # bg color for the left_frame
cf_bg = '#525b56' # bg color for the center_frame
fc_fg = "white" # text color for menu's properties
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=headlabelfont, bg='#2a3457', fg="white").pack(side=TOP, fill=X)
...
Label(left_frame, text="Name", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.375, rely=0.05)
Label(left_frame, text="Contact Number", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.175, rely=0.18)
Label(left_frame, text="Email Address", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.2, rely=0.31)
Label(left_frame, text="Gender", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.3, rely=0.44)
Label(left_frame, text="Date of Birth (DOB)", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.1, rely=0.57)
Label(left_frame, text="Stream", font=labelfont, bg=lf_bg, fg=fc_fg).place(relx=0.3, rely=0.7)
...
Button(left_frame, text='Submit and Add Record', font=labelfont, command=add_record, width=19, bg="green", fg=fc_fg).place(relx=0.035, rely=0.82)

# Placing components in the center frame
Button(center_frame, text='Delete Record', font=labelfont, command=remove_record,bg="red", width=15, fg=fc_fg).place(relx=0.1, rely=0.25)
Button(center_frame, text='View Record', font=labelfont, command=view_record,bg="blue",fg=fc_fg,  width=15).place(relx=0.1, rely=0.35)
Button(center_frame, text='Reset Fields', font=labelfont, command=reset_fields,bg="orange", fg=fc_fg, width=15).place(relx=0.1, rely=0.45)
Button(center_frame, text='Delete database', font=labelfont, command=reset_form, width=15, bg="red", fg=fc_fg).place(relx=0.1, rely=0.55)

# Placing components in the right frame
Label(right_frame, text='Students Records', font=headlabelfont, bg='#f2eaed', fg='black').pack(side=TOP, fill=X)
